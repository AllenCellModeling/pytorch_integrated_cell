...
#             p.data.clamp_(opt.clamp_lower, opt.clamp_upper)
        ...
        #pick a distribution that is obvious when you plot 
        x = Variable(dataProvider.get_images(next(inds_encD),'train'), requires_grad=True).cuda(gpu_id)
        zFake = enc(x)
        xHat = dec(zFake.detach())
        
        zReal = Variable(opt.latentSample(opt.batch_size, opt.nlatentdim), requires_grad=True).cuda(gpu_id)
        ...
        # torch.autograd.backward(errEncD_real, create_graph=True)        
        gradient_penalty = (grad(errEncD_real.mean(), zReal, create_graph=True)[0].norm() - 1).pow(2)
        ...
        # torch.autograd.backward(errEncD_fake, Variable(mone), create_graph=True)
        gradient_penalty = (grad(errEncD_fake.mean(), zFake, retain_graph=True, create_graph=True)[0].norm() - 1).pow(2)
        gradient_penalty.backward()
        ...
        gradient_penalty = (grad(errDecD_real.mean(), x, retain_graph=True, create_graph=True)[0].norm() - 1).pow(2)
        gradient_penalty.backward()

        errDecD_fake = decD(xHat)
        errDecD_fake.backward(mone, retain_variables=True)
        gradient_penalty = (grad(errDecD_fake.mean(), xHat, retain_graph=True, create_graph=True)[0].norm() - 1).pow(2)
        gradient_penalty.backward()
