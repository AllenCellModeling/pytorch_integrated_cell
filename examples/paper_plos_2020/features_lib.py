...
def save_feats(im, save_path, seg_method='local_mean', mask_intensity_features=True, stretch_intensity_features=False, save_imgs=False, figsize_hist=(30, 4), figsize_cells=(30, 4), dpi=100, debug=False):
    ...
    im_struct_unstretched = np.copy(im_tmp)
    if debug: print(f'im_struct.shape = {im_struct.shape}, im_struct_unstretched.shape = {im_struct_unstretched.shape}')
    ...
    if debug: print(f'im_struct.dtype = {im_struct.dtype}, im_struct_unstretched.dtype = {im_struct_unstretched.dtype}')
    ...
    if (stretch_intensity_features):
        cell_img_premask = im_struct[0]
        nuc_img_premask = im_struct[1]
    else:
        # No contrast stretching, but we still need to multiply all the images
        # with the same large constant value to convert the pixel intensities
        # to integers (most images have intensity values of 0 -> 1 coming out
        # from the model, although some will have values > 1)
        cell_img_premask = (im_struct_unstretched[0] * 255).astype('int')
        nuc_img_premask = (im_struct_unstretched[1] * 255).astype('int')
    ...
    if (mask_intensity_features):
        ...
        cell_img = np.where(cell_binmask[:, :, np.newaxis] > 0, cell_img_premask, 0)
        nuc_img = np.where(nuc_binmask[:, :, np.newaxis] > 0, nuc_img_premask, 0)
        ...
        #nuc_img = im_struct[1]
        cell_img = cell_img_premask
        nuc_img = nuc_img_premask
...
def fnGenRandomZ(argEmbeddingsParentPath, argModelDir, argRefSuffix, argBatchSize=1, argMode='train', debug=False):
    ...
    if debug: print(f'embeddings_unfiltered.shape = {embeddings_unfiltered.shape}')
...
def fnTraverseKey(argKey, argLevels, argSeparator='_'):
    return argSeparator.join(argKey.split(argSeparator)[:argLevels])

def fnExtractSubkey(argKey, argLevel, argSeparator='_'):
    return argKey.split(argSeparator)[argLevel]
...
def fnCalcPCA(argX, argColPrefix='pc_'):
    arrXStd = preprocessing.StandardScaler().fit_transform(argX)

    objPCA = PCA()
    arrPCs = objPCA.fit_transform(arrXStd)

    pca_colnames = [argColPrefix + str(dim) for dim in np.arange(arrPCs.shape[1])]

    dfPCs = pd.DataFrame(arrPCs, columns=pca_colnames)
    
    return dfPCs, objPCA

def fnCalcFeaturePCA(argDctCombinedDfFlat, argKey, argColPrefix='pc_', argCols=None, debug=False):
    if argCols is None:
        dfFeatures = argDctCombinedDfFlat[argKey]
        dfFeatures = dfFeatures.drop(['cell_idx'], axis=1)
    else:
        dfFeatures = argDctCombinedDfFlat[argKey][argCols]
        
    # Num rows with NaNs
    intNumRowsWithNaNs = dfFeatures.isna().any(axis=1).sum()
    if debug: print(f'intNumRowsWithNaNs = {intNumRowsWithNaNs}')

    arrFeatures = dfFeatures.to_numpy()
    arrFeaturesStd = preprocessing.StandardScaler().fit_transform(arrFeatures)

    objPCA = PCA()
    arrPCs = objPCA.fit_transform(arrFeaturesStd)
...
def fnFindTopPCs(argPCA, argCutoff=0.9, argDebug=False):
    
    fltTotalExpVar = 0
    
    for intDim, fltExpVar in enumerate(argPCA.explained_variance_ratio_):
        
        fltTotalExpVar = fltTotalExpVar + fltExpVar
        if (argDebug): print(f'{intDim}: {fltExpVar} -> {fltTotalExpVar}')
        
        if (fltTotalExpVar >= argCutoff):
            break
            
    return intDim + 1

def fnGenPrettyPCNames(argPCNames, argSeparator='_'):
    lstPrettyPCNames = []
    
    for strPCName in argPCNames:
        lstSplit = strPCName.split(argSeparator)
        lstPrettyPCNames.append(lstSplit[0].upper() + str((int(lstSplit[2]) + 1)))
        
    return lstPrettyPCNames

def fnGenPrettyFeatureNames(argFeatureNames):
    lstPrettyFeatureNames = []
    
    for strFeatureName in argFeatureNames:
        strFeatureName = strFeatureName.replace('dna_', 'nucleus_')
        strFeatureName = strFeatureName.replace('_shape_shape_', '_shape_')
        strFeatureName = strFeatureName.replace('_volume', '_area')
        strFeatureName = strFeatureName.replace('_surface_area', '_circumference')
        strFeatureName = strFeatureName.replace('_inten_', '_')
        strFeatureName = strFeatureName.replace('_std', '_std._dev.')
        strFeatureName = strFeatureName.replace('_', ' ')
        strFeatureName = strFeatureName.capitalize()
        lstPrettyFeatureNames.append(strFeatureName)
        
    return lstPrettyFeatureNames


# In[ ]:


# def fnSplitData(argData_X, argTarget_y, argValTestFrac=0.1, argSeed=1):
#     fltTrainTrac = 1 - argValTestFrac * 2
    
#     dctData_X = {}
#     dctData_X['train'] = argData_X.sample(frac=fltTrainTrac, argSeed)
#     dctData_X['test'] = 
#     arrTarget_
