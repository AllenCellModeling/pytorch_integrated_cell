...
class trainer(object):
    def __init__(self, dataProvider, n_channels, batch_size, n_latent_dim, n_classes, n_ref, gpu_ids):
        ...
        self.x = Variable(dataProvider.get_images(range(0, self.batch_size),'train').cuda(gpu_id))

        if self.n_ref > 0:
            self.ref = Variable(dataProvider.get_ref(range(0, self.batch_size), train_or_test='train').type_as(self.x.data).cuda(gpu_id))
        else:
            self.ref = None
    ...
    def iteration(self,
        ...
        if self.n_classes > 0:
            ...
            classes_one_hot = Variable((dataProvider.get_classes(inds,'train', 'one hot') - 1) * 25).type_as(zAll[c].data).cuda(self.gpu_ids[0])
    ...
    def save_state(self, enc, dec, decD,
                   optEnc, optDec, optDecD,
                   logger, zAll, opt):
