...
class DataProvider(DataProviderABC):
    def __init__(
        ...
        self.channel_lookup = [0, 1, 2]  # 0 means memb  # 1 means struct  # 2 means dna
        ...
        self.imsize = self.load_image(
            image_parent + os.sep + csv_df.iloc[0][self.image_col]
        ).shape
    ...
    def get_images(self, inds_tt, train_or_test):
        ...
        if self.crop_to is not None:
            ...
            images = images[
                :, :, crop_pre[0] : -crop_post[0], crop_pre[1] : -crop_post[1]  # noqa
            ]
