...
class DataProvider(DataProviderABC):
    ...
    def __init__(self, image_parent, csv_name='data_jobs_out.csv', opts={}, local_dir=None):
        self.data = {}
        
        opts_default = {'hold_out': 1/10,
                        'verbose': True,
        ...
        if self.opts['verbose']: print('Making h5 files')

        save_h5_reg_path_local = list()
        ...
        for index in tqdm(range(0, csv_df.shape[0]), desc='loading images into h5 files', ascii=True):
            ...
            if not os.path.exists(h5_path):
                ...
                try:
                    images = self.load_h5(h5_path, scale=False, use_lookup_table=False)
                    
                    [root, file] = os.path.split(h5_path)
                    [_, h5_dir] = os.path.split(root)
                    
                    h5_dir_local = local_dir + os.sep + h5_dir
                    h5_path_local = h5_dir_local + file
                    
                    if not os.path.exists(h5_dir_local):
                        os.makedirs(h5_dir_local)
                        
                    self.save_h5(images, h5_path_local)
                    
                    save_h5_reg_path_local.append(h5_path_local)
                except:
                    # pdb.set_trace()
        ...
        csv_df['save_h5_reg_path_local'] = save_h5_reg_path_local
    ...
    def load_h5(self, h5_path, scale = True, use_lookup_table = True):
        
        if use_lookup_table:
            chInds = self.channel_lookup_table[np.asarray(self.opts['channelInds'])]
        else:
            chInds = np.arange(0, len(self.channel_lookup_table))
    ...
    def save_h5(self, data, h5_path):
        h5f = h5py.File(h5_path, 'w')

        # h5 keys are the row numbers in the csv file
        h5f.create_dataset('image', data=data)
        h5f.close() 
    ...
    def get_images(self, inds_tt, train_or_test):
        ...
        for i, (rownum, row) in enumerate(self.csv_data.iloc[inds_master].iterrows()):
            h5_file = row.save_h5_reg_path_local
