...
if os.path.exists(fname) and not args.overwrite:
    ...
    for train_or_test in ["test", "train"]:
        ...
        iter_struct = [
            inds[j : j + opt.batch_size]  # noqa
            for j in range(0, len(inds), opt.batch_size)
        ]
...
if os.path.exists(fname) and not args.overwrite:
    ...
    for train_or_test in ["test", "train"]:
        ...
        for i in tqdm(iter_struct, desc="gen, " + train_or_test):
            ...
            classes = Variable(
                torch.Tensor(npts, opt.nClasses).fill_(-25).cuda(gpu_id), volatile=True
            )
            ...
            ref = Variable(
                torch.Tensor(npts, opt.nRef).normal_().cuda(gpu_id), volatile=True
            )
            struct = Variable(
                torch.Tensor(npts, opt.nRef).normal_().cuda(gpu_id), volatile=True
            )
...
for phase in dp.data.keys():
    ...
    for ind_phase, ind_master in enumerate(tqdm(dp.data[phase]["inds"])):
        ...
        data = [
            ...
            KL_divs_data[ind_phase],
            KL_divs_autoencode[ind_phase],
            KL_divs_gen[ind_phase],
        ]
...
for label in dp.label_names:
    ...
    inception_scores.append(
        [
            incept_data_train,
            incept_data_test,
            incept_autoencode_train,
            incept_autoencode_test,
            incept_gen_train,
            incept_gen_test,
        ]
    )
...
df_inception_scores = pd.DataFrame(
    inception_scores,
    index=list(dp.label_names) + ["all classes"],
    columns=[
        "data train",
        "data test",
        "autoencoded data train",
        "autoencoded data test",
