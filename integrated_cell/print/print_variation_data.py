...
for label_id in tqdm(label_ids, 'computing variation per class', ascii=True):
    ...
    class_test_inds = np.where(dp.get_classes(np.arange(0, n_test), 'test').numpy() == label_id)[0]
    im_test = dp.get_images(class_test_inds, 'test')
    
    class_train_inds = np.where(dp.get_classes(np.arange(0, n_train), 'train').numpy() == label_id)[0]
