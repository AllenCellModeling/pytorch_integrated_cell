...
for label_id, train_or_test in tqdm(job_list, 'computing errors', ascii=True):
    ...
    inds = np.where(dp.get_classes(np.arange(0, ndat), train_or_test).numpy() == label_id)[0]
    ...
    if args.use_current_results  or (os.path.exists(err_save_path) and not args.overwrite):
        continue
    ...
    inds_iter = [inds[j:j+opt.batch_size] for j in range(0, len(inds), opt.batch_size)]       
    
#     iter_struct = [inds[j:j+opt.batch_size] for j in range(0, len(inds_struct), opt.batch_size)]       
    # np.random.shuffle(data_iter)

    imgs_out_tmp = list()
    
    for j in range(0, len(inds_iter)):
        
        inds_ref_tmp = inds_iter[j]
        inds_struct_tmp = inds_iter[j]
        
        batch_size = len(inds_ref_tmp)
        
        embeddings_ref_tmp = embeddings_ref[train_or_test][inds_ref_tmp,:]
        embeddings_struct_tmp = embeddings_struct[train_or_test][inds_struct_tmp,:]
