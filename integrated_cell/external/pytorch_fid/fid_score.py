...
def calculate_fid_given_paired_paths(
    paths_real, paths_gen, batch_size=1, cuda=True, dims=2048
):
    """Calculates the FID of two paths"""
    ...
    fids = np.zeros(len(paths_real))

    for i, [path_real, path_gen] in enumerate(zip(paths_real, paths_gen)):
        m1, s1 = _compute_statistics_of_path(path_real, model, batch_size, dims, cuda)
        m2, s2 = _compute_statistics_of_path(path_gen, model, batch_size, dims, cuda)
