...
class DecD(nn.Module):
    def __init__(self, nClasses, nch, gpu_ids, opt=None):
        ...
        self.real_fake = nn.Linear(1024*int(self.fcsize), 1)
    
        if nclasses > 1:
            self.class_out = nn.Linear(1024*int(self.fcsize), nClasses)
    ...
    def forward(self, x):
        ...
        x_realfake = self.real_fake(x)
        x_out.append(x_realfake)
        
        if nclasses > 1:
            x_class = self.class_out(x)
            x_out.append(x_class)
