...
class DownLayer(nn.Module):
    def __init__(
        ...
        self.conv = spectral_norm(
            nn.Conv2d(ch_in, ch_out, ksize, dstep, padding=1, bias=False)
        )
...
class ResidualLayer(nn.Module):
    def __init__(
        ...
        self.resid = nn.Sequential(
            spectral_norm(nn.Conv2d(ch_in, ch_in, 3, 1, padding=1, bias=False)),
            nn.BatchNorm2d(ch_in),
            get_activation(activation),
            nn.AvgPool2d(2, stride=2, padding=0),
            spectral_norm(nn.Conv2d(ch_in, ch_out, 3, 1, padding=1, bias=False)),
