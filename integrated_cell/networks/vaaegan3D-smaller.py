...
class Enc(nn.Module):
    def __init__(self, nLatentDim, nClasses, nRef, nch, gpu_ids, activation='ReLU'):
        ...
        if self.nClasses > 0:
            self.classOut = nn.Sequential(
                nn.Linear(512*int(self.fcsize*1*1), self.nClasses),
        ...
        if self.nRef > 0:
            self.refOutMu = nn.Sequential(
                nn.Linear(512*int(self.fcsize*1*1), self.nRef))
        ...
        if self.nLatentDim > 0:
            self.latentOutMu = nn.Sequential(
                nn.Linear(512*int(self.fcsize*1*1), self.nLatentDim))
    ...
    def forward(self, x, reparameterize=False):
        ...
        x = x.view(x.size()[0], 512*int(self.fcsize*1*1))
...
class Dec(nn.Module):
    def __init__(self, nLatentDim, nClasses, nRef, nch, gpu_ids, activation='ReLU'):
        ...
        self.fc = nn.Linear(self.nLatentDim + self.nClasses + self.nRef, 512*int(self.fcsize*1*1))

        self.main = nn.Sequential(
            nn.BatchNorm3d(512),

            get_activation(activation),
            nn.ConvTranspose3d(512, 512, ksize, dstep, 1, output_padding = (0,1,0)),
            nn.BatchNorm3d(512),

            get_activation(activation),
            nn.ConvTranspose3d(512, 512, ksize, dstep, 1),
            nn.BatchNorm3d(512),
    ...
    def forward(self, xIn):
        ...
        x = self.fc(x)
        x = x.view(x.size()[0], 512, self.fcsize, 1, 1)
