...
class BasicLayer(nn.Module):
    def __init__(self, ch_in, ch_out, ksize=1, dstep=1, padding=0):
        super(BasicLayer, self).__init__()

        self.main = spectral_norm(
...
class DownLayer(nn.Module):
    def __init__(
        ...
        self.conv = spectral_norm(
            nn.Conv3d(ch_in, ch_out, ksize, dstep, padding=1, bias=True)
        )

        self.proj = None
        if ch_proj_in > 0:
            self.proj = BasicLayer(ch_proj_in, ch_out)

        self.proj_class = None
        if ch_proj_class > 0:
            self.proj_class = BasicLayer(ch_proj_class, ch_out)
...
class ResidualLayer(nn.Module):
    def __init__(
        ...
        self.resid = nn.Sequential(
            spectral_norm(nn.Conv3d(ch_in, ch_in, 3, 1, padding=1, bias=True)),
...
class Dec(nn.Module):
    def __init__(
        ...
        self.ref_fc = nn.Linear(self.n_ref, 1024 * int(self.fcsize * 1 * 1), bias=True)
        ...
        self.ref_path.append(
            UpLayer(
                l_sizes[i + 1], n_channels, activation_last="sigmoid", batch_norm=False
            )
        )
        self.target_path.append(
            UpLayer(
...
class DecD(nn.Module):
    def __init__(self, n_classes, n_channels, gpu_ids, noise_std, **kwargs):
        ...
        l_sizes = [n_channels, 64, 128, 256, 512, 512, 1024]
        for i in range(len(l_sizes) - 1):
            self.path.append(
                ResidualLayer(l_sizes[i], l_sizes[i + 1], activation="leakyrelu")
            )
