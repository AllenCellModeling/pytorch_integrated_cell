...
# like 9_4 but with
# Normal downconv on first layers of Enc and DecD
# Normal upconv on last layer of Dec
# 3x3 convolutions on the 2nd half of the residual path

# we're trying to save memory here with the vanilla deconv and upconv
...
class Enc(nn.Module):
    def __init__(
        ...
        self.ref_down = nn.Sequential(BasicLayer(n_channels, l_sizes[0]))
        self.target_down = nn.Sequential(BasicLayer(n_channels_target, l_sizes[0]))
    ...
    def forward(self, x, x_class):
        ...
        x_ref = self.ref_down(x_ref)
        x_target = self.target_down(x_target)
...
class Dec(nn.Module):
    def __init__(
        ...
        self.ref_bn_relu = spectral_norm(
            nn.BatchNorm3d(l_sizes[0]), nn.ReLU(inplace=True)
        )
        self.target_bn_relu = spectral_norm(
            nn.BatchNorm3d(l_sizes[0]), nn.ReLU(inplace=True)
        )
        ...
        self.ref_final = BasicLayerTranspose(
            l_sizes[i + 1], n_channels, activation="sigmoid", bn=False
        )

        self.target_final = BasicLayerTranspose(
            l_sizes[i + 1], n_channels_target, activation="sigmoid", bn=False
        )
    ...
    def forward(self, x_in):
        ...
        x_ref = self.ref_final(x_ref)
        x_target = self.target_final(x_target)
...
class DecD(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.path_down = nn.Sequential(BasicLayer(n_channels, self.l_sizes[0]))
        ...
        self.fc = spectral_norm(
            nn.Linear(
                self.l_sizes[-1] * int(np.prod(imsize_compressed)),
                n_classes,
                bias=False,
            )
        )
