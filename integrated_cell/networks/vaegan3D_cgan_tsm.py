...
import torch
from ..utils import spectral_norm, get_activation

from .common import TemporalShift
import numpy as np

# here we use a "temporal shift module" for the up/down residual paths, and 2D convs everywere else
# https://arxiv.org/pdf/1811.08383.pdf


class BasicLayer(nn.Module):
    def __init__(
        self, ch_in, ch_out, ksize=1, dstep=1, padding=0, activation="relu", bn=True
    ):
...
class DownLayerResidual(nn.Module):
    def __init__(
        ...
        self.resid = nn.Sequential(
            TemporalShift(tsm_nth_channel, n_shifts=tsm_n_shifts),
            spectral_norm(
                nn.Conv3d(
                    ch_in, ch_in, [4, 4, 1], stride=2, padding=[1, 1, 0], bias=True
                )
            ),
...
class UpLayerResidual(nn.Module):
    def __init__(
        ...
        self.resid = nn.Sequential(
            ...
            spectral_norm(
                nn.Conv3d(
                    ch_in, ch_out, [3, 3, 1], stride=1, padding=[1, 1, 0], bias=True
                )
            ),
            nn.BatchNorm3d(ch_out),
        )
        ...
        self.ref_path = nn.ModuleList(
            [
                DownLayerResidual(
                    ...
                    tsm_nth_channel=tsm_nth_channel,
                    tsm_n_shifts=tsm_n_shifts,
                )
            ]
        )
