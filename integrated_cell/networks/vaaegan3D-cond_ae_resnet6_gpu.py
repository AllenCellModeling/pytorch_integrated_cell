...
class Enc(nn.Module):
    ...
    def forward(self, x, x_class):
        ...
        x = x.cuda(gpu_ids[0])
        x_class = x_class.cuda(gpu_ids[0])
        ...
        x_ref = x_ref.view(x_ref.size()[0], 512 * int(self.fcsize * 1 * 1))
        x_target = x_target.view(x_target.size()[0], 512 * int(self.fcsize * 1 * 1))
        ...
        if self.gpu_out is not None:
            x = x.cuda(self.gpu_out)
...
class Dec(nn.Module):
    def __init__(
        ...
        self.ref_fc = nn.Linear(self.n_ref, 512 * int(self.fcsize * 1 * 1), bias=False)
        self.target_fc = nn.Linear(
            self.n_latent_dim, 512 * int(self.fcsize * 1 * 1), bias=False
        )
        self.ref_bn_relu = nn.Sequential(BatchNorm(512), nn.ReLU(inplace=True))
        self.target_bn_relu = nn.Sequential(BatchNorm(512), nn.ReLU(inplace=True))

        self.ref_path = nn.ModuleList([])
        self.target_path = nn.ModuleList([])

        l_sizes = [512, 512, 512, 256, 128, 64]
    ...
    def forward(self, x_in):
        ...
        x_class = x_class.cuda(gpu_ids[0])
        x_ref = x_ref.cuda(gpu_ids[0])
        x_target = x_target.cuda(gpu_ids[0])

        x_target = self.target_fc(x_target).view(
            x_target.size()[0], 512, self.fcsize, 1, 1
        )
        x_target = self.target_bn_relu(x_target)

        x_ref = self.ref_fc(x_ref).view(x_ref.size()[0], 512, self.fcsize, 1, 1)
        ...
        out = torch.zeros(out_shape).float()
        out = out.cuda(gpu_ids[0])

        out[:, self.ch_ref] = x_ref
        out[:, self.ch_target] = x_target

        if self.gpu_out is not None:
            out = out.cuda(self.gpu_out)

        return out


class DecD(nn.Module):
    def __init__(self, n_classes, n_channels, gpu_ids, noise_std, gpu_out=0, **kwargs):
        ...
        l_sizes = [n_channels, 64, 128, 256, 512, 512, 512]
        for i in range(len(l_sizes) - 1):
            self.path.append(
                DownLayer(l_sizes[i], l_sizes[i + 1], activation="leakyrelu")
            )

        self.fc = spectral_norm(nn.Linear(512 * int(self.fcsize), n_classes, bias=True))
    ...
    def forward(self, x_in):
        ...
        x_in = x_in.cuda(gpu_ids[0])

        if self.noise_std > 0:
            # allocate an appropriately sized variable if it does not exist
            noise = torch.zeros(x_in.size()).float().cuda(gpu_ids[2])
