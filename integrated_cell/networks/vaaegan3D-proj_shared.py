...
class ProjLayerFancy(nn.Module):
    def __init__(
        ...
        self.proj_conv = nn.DataParallel(
            spectral_norm(nn.Conv3d(ch_proj_in, ch_out, 1, 1, 0, bias=True))
        )
        self.proj_bn = nn.BatchNorm3d(ch_out)
        self.proj_activation = nn.LeakyReLU(0.2, inplace=True)

    def forward(self, x, x_class):
        x = self.conv(x)

        x_class = x_class.unsqueeze(2).unsqueeze(3).unsqueeze(4)

        x_class = self.proj_conv(x_class)
        x_class = self.proj_bn(x_class)
        x_class = self.proj_activation(x_class)

        x_class = x_class.expand(
...
class Enc(nn.Module):
    def __init__(
        ...
        if pretrained_path is not None:
            ...
            self.ref_path = nn.ModuleList([BasicLayer(n_channels, 32)])

            self.target_path = nn.ModuleList(
                [ProjLayer(n_channels_target, 32, n_classes)]
            )
    ...
    def __init__(
        ...
        if pretrained_path is not None:
            ...
            self.ref_bn_relu = nn.Sequential(
                nn.BatchNorm3d(1024), nn.ReLU(inplace=True)
            )
            ...
            l_sizes = [1024, 512, 256, 128, 64, 32]
            ...
            self.ref_path.append(
                nn.Sequential(
                    nn.DataParallel(
                        nn.ConvTranspose3d(
                            l_sizes[i + 1], n_channels, 4, 2, 1, bias=True
                        )
                    ),
                    nn.Sigmoid(),
                )
...
class DecD(nn.Module):
    def __init__(self, n_classes, n_channels, gpu_ids, noise_std, **kwargs):
        ...
        l_sizes = [n_channels, 32, 64, 128, 256, 512, 1024]
        for i in range(len(l_sizes) - 1):
            self.path.append(
                ProjLayerFancy(
                    l_sizes[i],
