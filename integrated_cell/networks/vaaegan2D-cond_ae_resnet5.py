...
# no residual on autoencoder output layer
...
class UpLayerProj(nn.Module):
    def __init__(
        ...
        super(UpLayerProj, self).__init__()
...
class Dec(nn.Module):
    def __init__(
        ...
        self.ref_path.append(
            UpLayerProj(l_sizes[i + 1], n_channels, activation_last="sigmoid")
        )
        self.target_path.append(
            UpLayerProj(
                l_sizes[i + 1],
