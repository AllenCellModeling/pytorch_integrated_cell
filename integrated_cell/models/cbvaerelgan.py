...
# conditional beta-vae relativistic gan
# Adapted from https://arxiv.org/abs/1807.00734
# https://github.com/AlexiaJM/RelativisticGAN/blob/master/code/GAN_losses_iter.py
...
class Model(bvaegan.Model):
    ...
    def iteration(self):
        ...
        for step_num in range(self.n_decD_steps):
            ...
            y = torch.ones(batch_size).type_as(x)
            ...
            # train with real
            yHat_xReal = decD(x, classes_onehot)
            yHat_xFake = decD(xHat.detach(), classes_onehot)
            ...
            yHat_xFake2 = decD(xHat.detach(), classes_onehot)

            err_decD = crit_decD(yHat_xReal - (yHat_xFake + yHat_xFake2) / 2, y)
            err_decD.backward()

            opt_decD.step()

            decDLoss += err_decD.item()
        ...
        yHat_xFake = decD(xHat, classes_onehot)

        yHat_xReal = decD(x, classes_onehot)

        minimaxDecDLoss = crit_decD(yHat_xFake - yHat_xReal, y)
        ...
        yHat_xFake2 = decD(xHat, classes_onehot[shuffle_inds])

        minimaxDecDLoss = crit_decD((yHat_xFake + yHat_xFake2) / 2 - yHat_xReal, y)
        minimaxDecDLoss.mul(self.lambda_decD_loss).backward()
        minimaxDecDLoss = minimaxDecDLoss.item()

        opt_dec.step()

        errors = [recon_loss, kld_loss_ref, kld_loss_struct, minimaxDecDLoss, decDLoss]
