...
class Model(bvaegan.Model):
    ...
    def iteration(self):
        ...
        for step_num in range(self.n_decD_steps):
            ...
            y_xReal = torch.zeros(x.shape[0], 1).type_as(x)
            y_xFake = torch.ones(x.shape[0], 1).type_as(x)

            with torch.no_grad():
                zAll = nn.parallel.data_parallel(enc, (x, classes_onehot), self.gpu_ids)
            ...
            with torch.no_grad():
                xHat = nn.parallel.data_parallel(
                    dec, ([classes_onehot] + zAll), self.gpu_ids
                )
            ...
            # train with real
            yHat_xReal = nn.parallel.data_parallel(
                decD, (x, classes_onehot), self.gpu_ids
            )
            ...
            yHat_xFake = nn.parallel.data_parallel(
                decD, (xHat, classes_onehot), self.gpu_ids
            )
            ...
            yHat_xFake2 = nn.parallel.data_parallel(
                decD, (xHat, classes_onehot), self.gpu_ids
            )
        ...
        # Forward passes
        z_ref, z_struct = nn.parallel.data_parallel(
            enc, (x, classes_onehot), self.gpu_ids
        )
        ...
        xHat = nn.parallel.data_parallel(dec, ([classes_onehot] + zAll), self.gpu_ids)
        ...
        shuffle_inds = torch.randperm(x.shape[0])
        xHat = nn.parallel.data_parallel(
            dec, ([classes_onehot[shuffle_inds]] + zAll), self.gpu_ids
        )

        yHat_xFake2 = nn.parallel.data_parallel(
            decD, (xHat, classes_onehot[shuffle_inds]), self.gpu_ids
        )
