...
class Model(bvaegan.Model):
    ...
    def iteration(self):
        ...
        for step_num in range(self.n_decD_steps):
            ...
            y_xReal = torch.ones(batch_size).type_as(x)
            y_xFake = torch.zeros(batch_size).type_as(x)
            with torch.no_grad():
            ...
            yHat_xFake2 = decD(xHat, classes_onehot)
        ...
        if self.penalize_mmDecD_loss > 0:
            # only works when we're using MSELoss and averaging over all data
            penalty = 1 - self.penalize_mmDecD_loss * decDLoss
            if penalty < 0:
                penalty = 0
        else:
            penalty = 1
