...
# like cbvae2_gan_ref2 but pass in X to each discriminator, and dont do decD(dec(z))
...
class Model(cbvae2_gan.Model):
    ...
    def iteration(self):
        ...
        for step_num in range(self.n_decD_steps):
            ...
            with torch.no_grad():
                zAll = enc(x, classes_onehot, x_mesh)
            ...
            # train with real
            yHat_xReal = decD(torch.cat([x, x, x_mesh], 1))
            errDecD_real = crit_decD(yHat_xReal, y_xReal)

            # train with fake, reconstructed
            yHat_xFake = decD(torch.cat([x, xHat, x_mesh], 1))
            ...
            yHat_xFake2 = decD(torch.cat([x, xHat, x_mesh], 1))
        ...
        if self.lambda_decD_loss > 0:
            ...
            yHat_xFake = decD(torch.cat([x, xHat, x_mesh], 1))
            minimaxDecLoss = crit_decD(yHat_xFake, y_xReal)

            # shuffle_inds = torch.randperm(x.shape[0])
            # xHat = dec([classes_onehot[shuffle_inds]] + zAll, x_mesh)

            # yHat_xFake2 = decD(torch.cat([xHat, x_mesh], 1))
