...
class Model(bvaegan.Model):
    ...
    def iteration(self):
        ...
        for step_num in range(self.n_decD_steps):
            ...
            # train with real
            yHat_xReal = nn.parallel.data_parallel(decD, x, self.gpu_ids)

            errDecD_real = crit_decD(yHat_xReal, y_xReal)

            # train with fake, reconstructed
            yHat_xFake = nn.parallel.data_parallel(decD, xHat, self.gpu_ids)
            ...
            yHat_xFake2 = nn.parallel.data_parallel(decD, xHat, self.gpu_ids)
